function distillation_breakeven
    ...
    for i = 1:10
        ...
        total_cost_function = Nf*(Qr+abs(Qc))*price_per_kJ...
...
%     price_for_0_profit = (Nv(N)*y_top(1)*Mw1*(cost_acetone/907.185) + cost_of_feed)/(Nf*(Qr+abs(abs(Qc)))+abs(Q_condense_final)+abs(Q_cooler_final))
    ...
    hold on
    title("Profit vs Feed Stream Position")
    xlabel("Feed Stream Position")
    ylabel("Profit ($USD)")
%     plot(price_for_0_profit,0,'ro')
