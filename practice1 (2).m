...
%     for i = 1:10
            ...
            elseif x_bot(1) >= 0.99
                ...
                % This calculates the total revenues from selling the top and bottom stream
            elseif x_bot(2) >= 0.99
                total_revenue_function = Nl(1)*x_bot(2)*Mw2*(cost_benzene/907.185)
                % This calculates the total revenues from selling the top and bottom stream
            elseif x_bot(3) >= 0.99
                total_revenue_function = Nl(1)*x_bot(3)*Mw3*(cost_toluene/907.185)
                % This calculates the total revenues from selling the top and bottom stream
            else
                profit_on_sold_liquid = NaN; 
                y_acetone = NaN;  % If the seperation of acetone/benzene isn't above 99%, we discard these points.
%                 continue
            end
            ...
            profit_on_sold_liquid = total_revenue_function - total_cost_function % Simply determines the difference in revenue and cost (i.e. Profit)        

    
    
%     breakeven = (total_revenue_function - heating_costs) / (Nf*(zf(1)*Mw1 + zf(2)*Mw2 + zf(3)*Mw3))
%     
%     plot(liquid_price, profit_on_sold_liquid)
%     grid on
%     
%     hold on
%     title("Profit vs Feed Market Rate")
%     xlabel("Feed Cost (USD/kg)")
%     ylabel("Profit (USD)")
