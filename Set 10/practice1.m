function distillation_breakeven
    ...
    for i = 1:10
            ...
            if y_top(1) >= 0.99
                total_revenue_function = Nv(N)*y_top(1)*Mw1*(cost_acetone/907.185)
            ...
            elseif y_top(3) >= 0.99
                ...
                % This calculates the total revenues from selling the top an421d bottom stream
            elseif x_bot(1) >= 0.99
                total_revenue_function = Nl(1)*x_bot(1)*Mw1*(cost_acetone/907.185)
                % This calculates the total revenues from selling the top and bottom stream
            elseif x_bot(2) >= 0.99
            ...
            % Q + liquid entering first stage - liquid leaving first stage - vapour
                % leaving first stage

            Q_bot = fsolve(@(Q) (Q + Nl(1) * (x_bot(1)*dHl(T(1), 1) + x_bot(2) * dHl(T(1), 2) + x_bot(3) * dHl(T(1), 3)) - ...
                                 Nl(2) * (x(2,1) * dHl(T(2), 1) + x(2,2) * dHl(T(2), 2) + x(2,3) * dHl(T(2), 3)) - ...
                                 Nv(1) * (y_bot(1) * dHv(T(1), 1) + y_bot(2) * dHv(T(1), 2) + y_bot(3) * dHv(T(1), 3)));

            % Q + vapour entering last stage - liquid leaving last stage - vapour
            % leaving last stage

            Q_top = fsolve(@(Q) (Q + Nv(N) * (y_top(1) * dHv(T(N), 1) + y_top(2) * dHv(T(N), 2) + y_top(3) * dHv(T(N), 3)) - ...
                                 Nl(N) * (x_top(N) * dHl(T(N-1), 1) + x_top(2) * dHl(T(N-1), 2) + x_bot(3) * dHl(T(N-1), 3)) - ...
                                 Nv(N-1) * (y(N-1,1) * dHv(T(N-1), 1) + y(N-1, 2) * dHv(T(N-1), 2) + y(N-1, 3) * dHv(T(N-1), 3)));
                                
            total_cost_function = (abs(Q_bot)+abs(Q_top))*price_per_kj...
                + price_per_kJ*abs(Q_condense_final)...
                + price_per_kJ*abs(Q_cooler_final)...
                + cost_of_feed
    ...
    F = linspace(1,10,10)
    plot(F, profit_on_sold_liquid)
    grid on
    
    hold on
    title("Profit vs Feed Tray Position")
    xlabel("Feed Tray Position")
    ylabel("Profit ($USD)")
%     t = sprintf("Breakeven cost is %.2f USD/ton", breakeven);
%     text(breakeven, 0.1, t)
%     text(breakeven - 80, -0.1, 'Breakeven cost of acetone')
    hold off
